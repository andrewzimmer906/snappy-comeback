class AddAttachmentImageToGifs < ActiveRecord::Migration
  def self.up
    add_column :gifs, :image_file_name, :string
    add_column :gifs, :image_content_type, :string
    add_column :gifs, :image_file_size, :integer
    add_column :gifs, :image_updated_at, :datetime
  end

  def self.down
    remove_column :gifs, :image_file_name
    remove_column :gifs, :image_content_type
    remove_column :gifs, :image_file_size
    remove_column :gifs, :image_updated_at
  end
end
